...
# Use TEST_NETWORK_HOME from parent (network.sh) or derive from script location
...
createChannel() {
	...
	while [ $rc -ne 0 -a $COUNTER -lt $MAX_RETRY ] ; do
		sleep $DELAY
		set -x
    . "${TEST_NETWORK_HOME}/scripts/orderer.sh" ${CHANNEL_NAME} > /dev/null 2>&1
    if [ $bft_true -eq 1 ]; then
...
joinChannel() {
  ORG=$1
  FABRIC_CFG_PATH="${FABRIC_CFG_PATH:-${TEST_NETWORK_HOME}/configtx}"
  setGlobals $ORG
	local rc=1
	local COUNTER=1
...
if [ $BFT -eq 1 ]; then
  FABRIC_CFG_PATH="${TEST_NETWORK_HOME}/bft-config"
fi
